...
router_users.post('/:id/follow', protect, async (req, res) => {
  try {
    if (req.params.id === req.user._id.toString()) return res.status(400).json({ success: false, message: "Can't follow yourself." });
    const [target, me] = await Promise.all([User.findById(req.params.id), User.findById(req.user._id)]);
    if (!target) return res.status(404).json({ success: false, message: 'User not found.' });
    const already = me.following.some(id => id.toString() === target._id.toString());
    if (already) { me.following.pull(target._id); target.followers.pull(me._id); me.followingCount = Math.max(0, me.followingCount-1); target.followersCount = Math.max(0, target.followersCount-1); }
    else { me.following.push(target._id); target.followers.push(me._id); me.followingCount++; target.followersCount++; const notif = await Notification.create({ recipient: target._id, sender: me._id, type: 'follow', message: me.username + ' started following you' }); const pop = await notif.populate('sender', 'username avatar verified'); req.app.get('io').to('user:' + target._id).emit('notification', pop); req.app.get('io').to('user:' + target._id).emit('new_follower', { from: { id: me._id, username: me.username, avatar: me.avatarUrl } }); }
